...
## [1.8.0] - 2026-09-04
### Added
- `analyze_measure_usage()` — for every measure, finds which visuals (by page + visual title/type) reference it in their field wells and which other measures reference it in their DAX. New "Measure Usage (Heuristic - Visuals & Cross-References)" dossier section listing this per measure, plus `analyze_unused_measures()` (derived from the same usage map) flagging measures with neither kind of reference in a companion "Unused Measures (Heuristic - Not in Any Visual or Other Measure)" section and an Executive Summary count. Both explicitly framed as heuristic leads, not a guarantee of dead code: they can't see filter-pane conditions, conditional formatting rules, or drillthrough filter targets, and only work for the modern PBIR report format (the legacy `Report/Layout` fallback never extracts per-visual fields at all, so every measure would falsely flag for a `.pbix` still on that format).
- Visual-usage matching is exact (after stripping a known aggregation-verb decoration like "Sum of " or a leading "Table." qualifier), not substring/word-boundary: a substring approach was tried first and rejected after testing surfaced a real false positive - `Cost Delta` matched as "used" inside a visual that only had `Cost Delta %`, since the space between them isn't an alphanumeric character and so doesn't stop a boundary-aware substring search. This is a common naming pattern (`Measure` + `Measure %`), so exact matching after decoration-stripping is the safer default.
